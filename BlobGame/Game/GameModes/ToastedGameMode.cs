...
internal sealed class ToastedGameMode : IGameMode {
    private const int HIGHEST_SPAWNABLE_BLOB_INDEX = 7;

    /// <summary>
    /// The id of the game mode. This must never change for a given game mode.
    /// </summary>
    public Guid Id { get; } = Guid.Parse("F56F2E5B-47C7-41ED-A542-184A607D04F8");
    ...
    public ToastedGameMode(int seed) {
    ...
    /// </summary>
    public void Load() {
        TextResource blobDataText = ResourceManager.TextLoader.Get("blobs_toasted");
    ...
    private Blob CreateBlob(Vector2 position, float rotation, int type) {
        ...
        //blob.Body.FixtureList.SingleOrDefault()?.BeforeCollision
    ...
    private bool OnBlobCollision(Fixture sender, Fixture other, Contact contact) {
        ...
        if (senderBlob != null && otherBlob != null) {
            ...
            Wall? wall = sender.Body.Tag is Wall w ? w : (other.Body.Tag is Wall w2 ? w2 : null);
